...
return function(camera)
	...
	function movement_system:process(entity, dt)
		...
		if not entity.possessed and entity.real_position and entity.real_orientation then
			local adjust = 0.1
			entity.position    = entity.position:lerp(entity.real_position, adjust)
			entity.orientation = entity.real_orientation--entity.orientation:lerp(entity.real_orientation, adjust)
		end
		...
		if false and entity.model and (not entity.bounds or entity.needs_update) then
			-- should be the bounding box triangles in world space
			assert(entity.model.bounds)
