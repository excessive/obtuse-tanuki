	...
	attribute vec4 v_bone; // NOTE: Actually used as int...

	uniform mat4 u_view, u_model, u_projection; // TODO: (ab)use love's built in matrices instead?
	uniform mat4 u_bone_matrices[200]; // this is why I want UBOs...
	...
	vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords) {
		...
		return vec4(texture2D(texture, texture_coords).rgb + ambient, 1.0) * color;
	}
#endif
...
varying vec2 f_uv_reflect;
	...
	vec4 shade(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords) {
		...
		if (u_shading != 2) {
			...
			if (u_shading == 4) return texture2D(u_map_Kd, f_uv) * color;
		}
